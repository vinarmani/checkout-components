...
class NodeClient extends Client {
  ...
  getCoinsByAddress(address, slp = false) {
    assert(typeof address === 'string');
    return this.get(`/coin/address/${address}?slp=${slp}`);
  }
  ...
  getCoinsByAddresses(addresses, slp = false) {
    assert(Array.isArray(addresses));
    return this.post(`/coin/address?slp=${slp}`, { addresses });
  }
  ...
  getCoin(hash, index, slp = false) {
    assert(typeof hash === 'string');
    assert((index >>> 0) === index);
    return this.get(`/coin/${hash}/${index}?slp=${slp}`);
  }
  ...
  getTXByAddress(address, slp = false) {
    assert(typeof address === 'string');
    return this.get(`/tx/address/${address}?slp=${slp}`);
  }
  ...
  getTXByAddresses(addresses, slp = false) {
    assert(Array.isArray(addresses));
    return this.post(`/tx/address?slp=${slp}`, { addresses });
  }
  ...
  getTX(hash, slp = false) {
    assert(typeof hash === 'string');
    return this.get(`/tx/${hash}?slp=${slp}`);
  }
